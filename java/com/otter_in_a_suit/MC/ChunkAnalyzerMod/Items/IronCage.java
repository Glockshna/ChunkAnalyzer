...
public class IronCage extends Item {
	 ...
	 * false if it don't. This is for ITEMS, not BLOCKS (only true experience
	 * can be ensured when there's a shitload of useless parameters none knows
	 * jack shit about)
	 */
	@Override
	...
	private int reduceExplosionLevel(int explosion_threshold, float by){
		...
		Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("Reduced the explosion-chance by 50%!"));
		return explosion_threshold;
	}
	
	/*
	MovingObjectPosition lastPosition = player.rayTrace(100, 1.0F);
	System.out.println("Pos: "+lastPosition.blockX + " " +
	lastPosition.blockY+ " " + lastPosition.blockZ);
	Bock block = world.getBlock(lastPosition.blockX,
	lastPosition.blockY, lastPosition.blockZ);
	Block block = world.getBlock(x, y, z);
	// System.out.println(block.getUnlocalizedName());
	if (block != null && block != Blocks.air
			&& block instanceof BaseScanner) {
		BaseScanner scanner = (BaseScanner) block;
